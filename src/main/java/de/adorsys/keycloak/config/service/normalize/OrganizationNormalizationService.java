...
public class OrganizationNormalizationService {
    ...
    private void normalizeOrganization(OrganizationRepresentation organization) {
        ...
        // Normalize identity provider configurations
        if (organization.getIdentityProviders() != null) {
            // Remove duplicates and nulls, normalize domains
            List<OrganizationIdentityProviderConfig> uniqueIdps = new ArrayList<>();
            Set<String> seen = new HashSet<>();
            
            for (OrganizationIdentityProviderConfig idpConfig : organization.getIdentityProviders()) {
                if (idpConfig != null && StringUtils.hasText(idpConfig.getAlias()) 
                        && seen.add(idpConfig.getAlias().trim())) {
                    // Normalize the alias
                    idpConfig.setAlias(idpConfig.getAlias().trim());
                    
                    // Normalize the domain to lowercase
                    if (idpConfig.getDomain() != null) {
                        idpConfig.setDomain(idpConfig.getDomain().toLowerCase().trim());
                    }
                    
                    uniqueIdps.add(idpConfig);
                }
            }
